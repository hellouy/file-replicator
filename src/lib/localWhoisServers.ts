/**
 * 本地 WHOIS 服务器列表补充
 * 用于补充 Supabase 中的 WHOIS 服务器数据
 * 包含一些云端列表可能缺失或不可用的 TLD
 */

export const LOCAL_WHOIS_SERVERS: Record<string, string> = {
  // ==================== 非洲国家代码 TLD ====================
  ...
  'gu': 'whois.nic.gu',        // 关岛 Guam
  'mh': 'whois.nic.mh',        // 马绍尔群岛 Marshall Islands
  'mp': 'whois.nic.mp',        // 北马里亚纳群岛 Northern Mariana Islands
  'as': 'whois.nic.as',        // 美属萨摩亚 American Samoa
  ...
  'gp': 'whois.nic.gp',        // 瓜德罗普 Guadeloupe
  'mq': 'whois.nic.mq',        // 马提尼克 Martinique
  'ms': 'whois.nic.ms',        // 蒙特塞拉特 Montserrat
  'bq': 'whois.nic.bq',        // 博内尔岛 Bonaire
  'cw': 'whois.nic.cw',        // 库拉索 Curacao
  'sx': 'whois.nic.sx',        // 圣马丁 Sint Maarten
  
  // ==================== 其他特殊TLD ====================
  'eh': 'whois.nic.eh',        // 西撒哈拉 Western Sahara
 ...
 * 获取 WHOIS 服务器地址
 * @param tld - 顶级域名（不带点）
 * @param supabaseServer - 来自 Supabase 的服务器地址
 * @returns WHOIS 服务器地址，优先使用本地列表
 */
export function getWhoisServer(tld: string, supabaseServer?: string | null): string | null {
  const cleanTld = tld.toLowerCase().replace(/^\./, '');
  
  // 优先使用本地补充列表（可以覆盖 Supabase 中的版本）
  if (LOCAL_WHOIS_SERVERS[cleanTld]) {
    return LOCAL_WHOIS_SERVERS[cleanTld];
  }
...
/**
 * 检查 TLD 是否有本地 WHOIS 服务器
 */
export function hasLocalWhoisServer(tld: string): boolean {
  const cleanTld = tld.toLowerCase().replace(/^\./, '');
  return cleanTld in LOCAL_WHOIS_SERVERS;
}

/**
 * 获取所有本地支持的 TLD 列表
 */
export function getLocalTlds(): string[] {
  return Object.keys(LOCAL_WHOIS_SERVERS);
}
